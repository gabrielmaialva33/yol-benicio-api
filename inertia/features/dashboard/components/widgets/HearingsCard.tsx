...
export function HearingsCard() {
  ...
  const overallPercentage =
    totalHearings > 0 ? Math.round((totalCompleted / totalHearings) * 100) : 0
      ...
      <CardContent className="space-y-5 relative z-10">
        {hearings.length === 0 ? (
          ...
          hearings.map((item, index) => {
            ...
            return (
              <div
                className="group/item p-4 rounded-xl bg-gradient-to-r from-gray-50/50 to-white border border-gray-100 hover:border-gray-200 transition-all duration-200"
                key={item.label}
              >
                <div className="flex items-center gap-4">
                  <div className="flex-shrink-0">
                    <div className={`p-3 ${colorScheme.light} rounded-xl`}>
                      <IconComponent
                        className={`h-6 w-6 ${colorScheme.bg.replace('bg-', 'text-')}`}
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div
                          className={`px-2 py-1 rounded-full text-xs font-medium ${colorScheme.light} ${colorScheme.bg.replace('bg-', 'text-')}`}
                        >
                          {item.percentage >= 80
                            ? 'Excelente'
                            : item.percentage >= 60
                    ...
                    <div className="flex justify-between text-sm text-gray-600 mb-3">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          Cumpridos:{' '}
                          <span className="font-medium text-emerald-600">{item.completed}</span>
                        </span>
                      </div>
                      <div className="flex items-center gap-1">

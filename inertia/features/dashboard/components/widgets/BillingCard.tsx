...
export function BillingCard() {
  ...
  const percentageIcon =
    billingData?.percentage && billingData.percentage > 0 ? (
      ...
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <title>Down</title>
        <path d="M19 7l-10 10-5-5" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
      </svg>
    )
  ...
  return (
    <Card tinted={true}>
      <CardHeader className="flex items-start justify-between mb-4">
        <CardTitle className="text-[var(--color-text-primary)]">Faturamento</CardTitle>
        <div className="text-right">
          <div className={`flex items-center space-x-1 font-semibold ${percentageColor}`}>
            {percentageIcon}
      ...
      <div className="text-[40px] font-bold mb-4 leading-none">{billingData?.value || 'R$ 0'}</div>
      <CardContent className="h-16 -mx-6 -mb-6">
        <ResponsiveContainer height="100%" width="100%">
          <LineChart data={billingData?.chart || []}>
            <Line dataKey="pv" dot={false} stroke="#004B50" strokeWidth={2} type="monotone" />
          </LineChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>

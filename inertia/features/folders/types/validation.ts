...
const partySchema = z.object({
  ...
  cpf: z.string()
    .optional()
    .refine(
      (cpf) => !cpf || cpfRegex.test(cpf),
      { message: 'CPF inválido. Use o formato 000.000.000-00' }
    ),
  cnpj: z.string()
    .optional()
    .refine(
      (cnpj) => !cnpj || cnpjRegex.test(cnpj),
...
export const folderSchema = z.object({
  ...
  processNumber: z.string()
    ...
    .refine(
      (num) => processNumberRegex.test(num),
  ...
  cnjNumber: z.string()
    .optional()
    .refine(
      (num) => !num || cnjNumberRegex.test(num),
  ...
  folder: z.string()
    ...
    .max(100, 'Nome da pasta muito longo'),
  ...
  organ: z.string()
    ...
    .max(255, 'Nome do órgão muito longo'),
  ...
  internalCode: z.string()
    .max(50, 'Código interno muito longo')
    .optional(),
  
  searchType: z.string()
    .max(100, 'Tipo de busca muito longo')
    .optional(),
  ...
  judge: z.string()
    .max(255, 'Nome do juiz muito longo')
    .optional(),
  ...
  subArea: z.string()
    .min(1, 'Subárea é obrigatória')
    .max(100, 'Nome da subárea muito longo'),
  
  core: z.string()
    .max(100, 'Núcleo muito longo')
    .optional(),
  
  district: z.string()
    .min(1, 'Comarca é obrigatória')
    .max(100, 'Nome da comarca muito longo'),
  
  court: z.string()
    .min(1, 'Tribunal é obrigatório')
    .max(255, 'Nome do tribunal muito longo'),
  
  courtDivision: z.string()
    .max(100, 'Vara muito longa')
    .optional(),
  
  partner: z.string()
    .min(1, 'Sócio responsável é obrigatório')
    .max(255, 'Nome muito longo'),
  
  coordinator: z.string()
    .max(255, 'Nome do coordenador muito longo')
    .optional(),
  ...
  objectDetail: z.string()
    .max(1000, 'Detalhe do objeto muito longo')
    .optional(),
...
export const validateFolder = (data: unknown): { success: true; data: FolderFormData } | { success: false; errors: Record<string, string> } => {
